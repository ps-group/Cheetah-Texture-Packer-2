...
QPoint MaxRects::insertNode(InputImage* input)
{
	...
	for (i = 0; i < F.size(); i++)
    {
		if ((F.at(i).r.width() >= img.width() && F.at(i).r.height() >= img.height()) || (F.at(i).r.width() >= img.height() && F.at(i).r.height() >= img.width()))
        {
            rotated = false;
            m = 0;
			if ((F.at(i).r.width() >= img.height() && F.at(i).r.height() >= img.width()) && !(F.at(i).r.width() >= img.width() && F.at(i).r.height() >= img.height()))
            {
				if (rotation == 0)
                {
                    continue;
            ...
            case ImagePacker::TL:
                ...
                {
					if (qAbs(R.at(k).y() + R.at(k).height() / 2 - F.at(i).r.y() - F.at(i).r.height() / 2) < qMax(R.at(k).height(), F.at(i).r.height()) / 2)
                    {
