...
void ImagePacker::realculateDuplicates()
{
	...
	for (int i = 0; i < images.count(); i++)
    {
        ...
        {
			if (images.at(k).duplicateId == NULL && images.at(i).hash == images.at(k).hash && images.at(i).size == images.at(k).size && images.at(i).crop == images.at(k).crop)
            {
...
void ImagePacker::SortImages(int w, int h)
{
	...
	for (int i = 0; i < images.size(); i++)
    {
		...
		images.operator[](i).rotated = false;
		if ((rotate == WIDTH_GREATHER_HEIGHT && size.width() > size.height()) || (rotate == WIDTH_GREATHER_2HEIGHT && size.width() > 2 * size.height()) || (rotate == HEIGHT_GREATHER_WIDTH && size.height() > size.width()) || (rotate == H2_WIDTH_H && size.height() > size.width() && size.width() * 2 > size.height()) || (rotate == W2_HEIGHT_W && size.width() > size.height() && size.height() * 2 > size.width()) || (rotate == HEIGHT_GREATHER_2WIDTH && size.height() > 2 * size.width()))
        {
            size.transpose();
			images.operator[](i).rotated = true;
        }
